...
export default function simulate(resolution: number, gravity: number, maxX: number, maxY: number, objects:Array<Object>, types: TypeDictionary) {
    for (let index = 0; index < objects.length; index++) {
        ...
        for (let index = 0; index < objects.length; index++) {
            ...
            if (distanceX < resolution && distanceY < resolution) {
                ...
                otherObject.velocity.x = otherObject.velocity.x * otherObjectType.restitution - vectorX
                otherObject.velocity.y = otherObject.velocity.y * otherObjectType.restitution - vectorY
            }
